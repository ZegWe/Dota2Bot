...
class Fortune(Plugin):
    ...
    def __init__(self, group_id: int, sender: GroupSender):
        ...
        self.commands.append(
            Command(['今日运势', '今天啥运气'], [], '： 显示今日的Dota2运势', self.get_fortune))
        self.commands.append(Command(['幸运英雄', '今天玩点啥', '不知道玩啥', '玩啥好呢'], [
        ], '： 显示今日的Dota2幸运英雄', self.get_hero))
    ...
    def get_hero(self, FromUserId: int) -> None:
        ...
        m = '[ATUSER({})]的今日幸运英雄是{}，{}'.format(
            FromUserId, tmp_user.hero.hero, tmp_user.hero.sentence)
        self.sender.send(m)
    ...
    def get_fortune(self, FromUserId: int) -> None:
        if FromUserId in self.users:
            ...
            if not tmp_user.fort or tmp_user.fort.upd_date != get_date():
                ...
                tmp_user = User(Fort(tmp, random.choice(
                    fortuneDict[tmp])), tmp_user.hero)
                self.users[FromUserId] = tmp_user
        else:
            tmp = random.choice([*fortuneDict])
