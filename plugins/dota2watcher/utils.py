...
def get_last_match_id_by_short_steamID(short_steamID: int) -> int:
    ...
    url = 'https://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v1?key={}' \
        '&account_id={}&matches_requested=1'.format(
            Config.api_key, short_steamID)
    try:
    ...
    if response.status_code >= 400:
        if response.status_code == 401:
            logger.error(DOTA2HTTPError(
                "Unauthorized request 401. Verify API key.", url))
            return -1
        if response.status_code == 429:
...
def generate_message(match_id: int, accounts: list[Account]) -> list[str]:
    ...
    team_damage = sum(
        [x.damage for x in detail.players if x.radiant == radiant])
    team_kills = sum([x.kills for x in detail.players if x.radiant == radiant])
    team_deaths = sum(
        [x.deaths for x in detail.players if x.radiant == radiant])
    ...
    for player in players:
        ...
        participation = 0 if team_kills == 0 else (
            100 * float(kills + assists) / team_kills)
        deaths_rate = 0 if team_deaths == 0 else (
            100 * float(deaths) / team_deaths)
        print_str += "\n\n{}使用{}\nKDA: {:.2f}[{}/{}/{}]\nGPM/XPM: {}/{}\n补刀数: {}/{}\n总伤害: {}({:.2f}%)\n参战率: {:.2f}%\n参葬率: {:.2f}%" \
